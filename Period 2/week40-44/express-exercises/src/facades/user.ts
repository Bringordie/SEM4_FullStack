...
export default class UserFacade {
  ...
  // public static users:Array<IGameUser> = makeData()
  ...
  static validateClass(res: any) {
    const schema = {
      name: Joi.string().min(3).required(),
      userName: Joi.string().min(3).required(),
      password: Joi.string().min(8).required() // Could also use https://www.npmjs.com/package/joi-password-complexity
    };
...
async function test() {
  ...
  //Should fail
  try {
    const ok = await UserFacade.checkUser("admin@a.dk", "secret");
    debug("Password and user was OK", ok);
    await UserFacade.checkUser("admin@a.dk", "wrong password");
    debug("I should not get here");
  } catch (err) {
    debug("Password did not match");
  }
